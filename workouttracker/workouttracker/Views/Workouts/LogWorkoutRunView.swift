...
struct LogWorkoutRunView: View {
    ...
    func saveWorkout() {
        ...
        db.collection("users").document(uid).collection("run").addDocument(data: data) { err in
            if let err = err {
                message = "Error: \(err.localizedDescription)"
            } else {
                message = "Run workout saved!"
